...
class GaitSequenceDataset(data.Dataset):
    ...
    def _load_data(self):
        ...
        for i in range(1, 2):
            ...
            midi_file_path_left_base = os.path.join(self.root_dir, folder_name, 'midis', folder_name+'_6_min_walking_test_leftseq_')
            midi_file_path_right_base = os.path.join(self.root_dir, folder_name, 'midis', folder_name+'_6_min_walking_test_rightseq_')
            ...
            if len(filenames) > 0:
                for f in filenames:
                    if f.split('.')[-1] == 'h5':
                        ...
                        for i in range(1, len(left_leg_sequences)):
                            sequence = left_leg_sequences[i]
                            # Consider only the valid sequences
                            if left_turn_segments[sequence[0]] != 1:
                                seq = flexion_angles[sequence[0]:sequence[1]]
                                seq_len = len(seq)
                                if seq_len >= self.shortest_sequence and seq_len <= self.longest_sequence:
                                    # Padding sequences to make the length equal to longest_sequence
                                    pad = np.array([[0, 0, 0, 0, 0, 0]]*(self.longest_sequence-seq_len))
                                    if len(pad) > 0:
                                        seq = np.append(seq, pad, 0)
                                    self.sequences.append(seq)
